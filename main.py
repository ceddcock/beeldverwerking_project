...
import numpy as np
#filepath = "../git/data/reference-dataset/translation/img/" # path to images
...
coef1, coef2 = reg.get_swt_coeffs(img1_gray,downsample=downsample), reg.get_swt_coeffs(img2_gray,downsample=downsample)
level_1 = 1
level_2 = 1
coef1_L = coef1[0]
coef2_L = coef2[0]
LH1,HL1 = coef1[level_1][0], coef1[level_1][1]
LH2,HL2 = coef2[level_2][0], coef2[level_2][1]
...
while len(coords_orig) < 100 and level_1 <= 3:
    level_1 += 1
    LH1, HL1 = coef1[level_1][0], coef1[level_1][1]
    coords_orig_LH = list(corner_peaks(corner_harris(LH1), threshold_rel=Harris_th_rel, min_distance=Harris_min_dist))
    coords_orig_HL = list(corner_peaks(corner_harris(HL1), threshold_rel=Harris_th_rel, min_distance=Harris_min_dist))
    ...
    for c in coords_orig_LH:
        coords_orig.append(c)
while len(coords_warped) < 100 and level_2 <= 3:
    level_2 += 2
    LH2, HL2 = coef2[level_2][0], coef2[level_2][1]
    coords_warped_LH = list(corner_peaks(corner_harris(LH2), threshold_rel=Harris_th_rel, min_distance=Harris_min_dist))
    coords_warped_HL = list(corner_peaks(corner_harris(HL2), threshold_rel=Harris_th_rel, min_distance=Harris_min_dist))
...
src, dst = find_matches(coords_orig, coords_warped, coef1_L, coef2_L,match_max_dist=200,window_ext=5)
